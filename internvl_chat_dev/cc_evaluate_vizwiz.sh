...
RESULT_ROOT="/mnt/petrelfs/wangwenhai/workspace/InternVL-release/internvl_chat_dev/cc_results/vizwiz/"
CKPT_FNAMES=(
    "checkpoint-13000"
)
mkdir -p $RESULT_ROOT
...
#        --batch_size 1 --shots 0 --datasets vizwiz --chat-few-shot-style multi \
...
#        --cpus-per-task=2 --kill-on-bad-exit=1 --quotatype=auto \
#        bash run_eval.sh --model internvl_chat \
#        --batch_size 1 --shots 1 --datasets vizwiz --chat-few-shot-style multi \
#        --load_in_8bit False --dynamic False --max_num 6 \
#        --checkpoint $CKPT_ROOT/$CKPT_FNAME \
...
#        --cpus-per-task=2 --kill-on-bad-exit=1 --quotatype=auto \
#        bash run_eval.sh --model internvl_chat \
#        --batch_size 1 --shots 2 --datasets vizwiz --chat-few-shot-style multi \
#        --load_in_8bit False --dynamic False --max_num 6 \
#        --checkpoint $CKPT_ROOT/$CKPT_FNAME \
#        --results_file "$RESULT_ROOT/results_${CKPT_FNAME}_2shots-multi-rounds.result" &
    srun -p VC2 --gres=gpu:8 -N 1 \
        --cpus-per-task=2 --kill-on-bad-exit=1 --quotatype=auto \
        bash run_eval.sh --model internvl_chat \
        --batch_size 1 --shots 4 --datasets vizwiz --chat-few-shot-style multi \
        --load_in_8bit False --dynamic False --max_num 6 \
        --checkpoint $CKPT_ROOT/$CKPT_FNAME \
...
#        --cpus-per-task=2 --kill-on-bad-exit=1 --quotatype=auto \
#        bash run_eval.sh --model internvl_chat \
#        --batch_size 1 --shots 8 --datasets vizwiz --chat-few-shot-style multi \
#        --load_in_8bit False --dynamic False --max_num 6 \
#        --checkpoint $CKPT_ROOT/$CKPT_FNAME \
