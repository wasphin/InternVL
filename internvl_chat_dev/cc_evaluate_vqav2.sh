...
RESULT_ROOT="/mnt/petrelfs/wangwenhai/workspace/InternVL-release/internvl_chat_dev/cc_results/vqav2/"
CKPT_FNAMES=(
    "checkpoint-13000"
)
mkdir -p $RESULT_ROOT
...
#        --batch_size 1 --shots 0 --datasets vqav2 --chat-few-shot-style multi \
...
#        --cpus-per-task=2 --kill-on-bad-exit=1 --quotatype=auto \
#        bash run_eval.sh --model internvl_chat \
#        --batch_size 1 --shots 1 --datasets vqav2 --chat-few-shot-style multi \
#        --load_in_8bit False --dynamic False --max_num 6 \
#        --checkpoint $CKPT_ROOT/$CKPT_FNAME \
...
#        --cpus-per-task=2 --kill-on-bad-exit=1 --quotatype=auto \
#        bash run_eval.sh --model internvl_chat \
#        --batch_size 1 --shots 2 --datasets vqav2 --chat-few-shot-style multi \
#        --load_in_8bit False --dynamic False --max_num 6 \
#        --checkpoint $CKPT_ROOT/$CKPT_FNAME \
...
#        --cpus-per-task=2 --kill-on-bad-exit=1 --quotatype=auto \
#        bash run_eval.sh --model internvl_chat \
#        --batch_size 1 --shots 4 --datasets vqav2 --chat-few-shot-style multi \
#        --load_in_8bit False --dynamic False --max_num 6 \
#        --checkpoint $CKPT_ROOT/$CKPT_FNAME \
#        --results_file "$RESULT_ROOT/results_${CKPT_FNAME}_4shots-multi-rounds.result" &
    srun -p VC2 --gres=gpu:8 --ntasks=10 --ntasks-per-node=1  \
        --cpus-per-task=2 --kill-on-bad-exit=1 --quotatype=auto \
        bash run_eval.sh --model internvl_chat \
        --batch_size 1 --shots 8 --datasets vqav2 --chat-few-shot-style multi \
        --load_in_8bit False --dynamic False --max_num 6 \
        --checkpoint $CKPT_ROOT/$CKPT_FNAME \
